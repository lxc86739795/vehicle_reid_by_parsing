...
class VehicleID_Small_Mask(ImageMaskDataset):
    """
    vehicleid
    Reference:
    ...
    def __init__(self, root='/home/liuxinchen3/notespace/data', verbose=True, **kwargs):
        ...
        self.mask_dir = osp.join(self.dataset_dir, 'image_mask')
        self.train_list = osp.join(self.dataset_dir, 'train_test_split/train_list_3000.txt')
        self.test_list = osp.join(self.dataset_dir, 'train_test_split/test_list_3000.txt')
        ...
        super(VehicleID_Small_Mask, self).__init__(train, query, gallery, **kwargs)
    ...
    def _process_dir(self, list_file, relabel=False):
        ...
        for idx, line in enumerate(img_list_lines):
            ...
            dataset.append((img_path, mask_path, int(vid), int(imgid)))
        ...
        if relabel:
            ...
            for sample in dataset:
                if sample[2] not in vid_container:
                    vid_container.add(sample[2])
                    query.append(sample)
                else:
