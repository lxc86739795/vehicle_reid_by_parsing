...
class VeRiWild_Medium_Mask(ImageMaskDataset):
    """
    VeRi-Wild
    Reference:
    ...
    def __init__(self, root='/home/liuxinchen3/notespace/data', verbose=True, **kwargs):
        ...
        self.mask_dir = osp.join(root, self.dataset_dir, 'images_mask')
        self.train_list = osp.join(root, self.dataset_dir, 'train_test_split_medium/train_3500.txt')
        self.query_list = osp.join(root, self.dataset_dir, 'train_test_split_medium/test_1500_query.txt')
        self.gallery_list = osp.join(root, self.dataset_dir, 'train_test_split_medium/test_1500.txt')
        ...
        self.imgid2vid, self.imgid2camid, self.imgid2imgpath, self.imgid2maskpath = self._process_vehicle(self.vehicle_info)
        ...
        super(VeRiWild_Medium_Mask, self).__init__(train, query, gallery, **kwargs)
    ...
    def _process_dir(self, img_list, relabel=False):
        ...
        for idx, line in enumerate(img_list_lines):
            ...
            dataset.append((self.imgid2imgpath[imgid], self.imgid2maskpath[imgid], int(vid), int(self.imgid2camid[imgid])))
    ...
    def _process_vehicle(self, vehicle_info):
        ...
        for idx, line in enumerate(vehicle_info_lines[1:]):
            ...
            mask_path = osp.join(self.mask_dir, vid, imgid + '.png')
            
            imgid2vid[imgid] = vid
            imgid2camid[imgid] = camid
            imgid2imgpath[imgid] = img_path
            imgid2maskpath[imgid] = mask_path
            # print(idx, vid, imgid, camid, img_path)

        assert len(imgid2vid) == len(vehicle_info_lines)-1
        return imgid2vid, imgid2camid, imgid2imgpath, imgid2maskpath
