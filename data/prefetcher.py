...
class instance_prefetcher():
    ...
    def preload(self):
        try:
            self.next_input, self.next_target, self.next_camid, self.next_idx = next(self.loader)
        except StopIteration:
            self.next_input = None
            self.next_target = None
            self.next_camid = None
    ...
    def next(self):
        ...
        if target is not None:
            target.record_stream(torch.cuda.current_stream())
        self.preload()
        return input, target, camid, index
...
class data_prefetcher_mask(): # Changed by Xinchen Liu
    ...
    def preload(self):
        try:
            self.next_input, self.next_mask, self.next_target, self.next_camid = next(self.loader)
        except StopIteration:
            self.next_input = None
            self.next_mask = None
            self.next_target = None
